...
def main() -> None:
    ...
    temporary = Path(tempfile.mkdtemp(prefix='final-1.0-', dir=BASE)).resolve()
    ...
    try:
        public = SETUPS / 'HALVETH-Morrowind-Genesis-1.0.0-Setup.exe'
        private = SETUPS / 'HALVETH-Morrowind-Genesis-1.0.0-Local-Engine-Setup.exe'
        ...
        receipt = {'schema': 'halveth.genesis.installer-smoke/1', 'version': '1.0.0',
        ...
        (BASE / 'final-1.0-receipt.json').write_text(json.dumps(receipt, indent=2) + '\n',
                                                   encoding='utf-8')
